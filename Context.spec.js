...
describe("CCBLContextState<boolean, ?, ?> creation:", () => {
    ...
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        state: new StateInExecutionEnvironment_1.CCBLStateInExecutionEnvironment({ env, expression: "false", stateName: "pipo" })
    });
...
describe("CCBLContextState<number, ?, ?> creation:", () => {
    const env = new ExecutionEnvironment_1.CCBLEnvironmentExecution(clock);
    const pipoNum = new EmitterValue_1.CCBLEmitterValue(10);
    env.register_CCBLEmitterValue("pipoNum", pipoNum);
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        state: new StateInExecutionEnvironment_1.CCBLStateInExecutionEnvironment({ env, expression: "pipoNum > 20", stateName: "pipo" })
    });
    it("Context is not activable", () => expect(context.getActivable()).toBe(false));
    it("Context is not activated", () => expect(context.getActive()).toBe(false));
    it("State activation has no effect on context", () => {
...
describe("CCBLContextState<?, boolean, boolean> creation:", () => {
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        eventStart: new Event_1.CCBLEvent({ eventName: "start", env }),
        eventFinish: new Event_1.CCBLEvent({ eventName: "finish", env })
    });
    it("Context is not activable", () => expect(context.getActivable()).toBe(false));
    it("Context is not activated", () => expect(context.getActive()).toBe(false));
...
describe("CCBLContextState<?, evt, evt>:", () => {
    let evt = new Event_1.CCBLEvent({ eventName: "start", env });
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        eventStart: evt,
        eventFinish: evt
...
describe("CCBLContextState<?, evtS, evtF>:", () => {
    let evtS = new Event_1.CCBLEvent({ eventName: "start", env });
    let evtF = new Event_1.CCBLEvent({ eventName: "finish", env });
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        eventStart: evtS,
        eventFinish: evtF
    ...
    it("If evtE and evtF trigger at the same time, the context should remains active", () => {
        evtS.trigger({ value: true });
        evtF.trigger({ value: true });
        expect(context.getActive()).toBe(true);
    });
});
...
describe("CCBLContextState<?, number, number> (Hysteresis 18-20):", () => {
    ...
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        eventStart: new Event_1.CCBLEvent({ eventName: "start", env, expressionFilter: "event.value < 18" }),
        eventFinish: new Event_1.CCBLEvent({ eventName: "finish", env, expressionFilter: "event.value > 20" })
    });
    context.onActiveUpdated(activeValue => nbUpdate++);
    context.setActivable(true);
...
describe("CCBLContextState<number, number, number> unactivated:", () => {
    ...
    let context = new ContextState_1.CCBLContextState({
        environment: env,
        state: new StateInExecutionEnvironment_1.CCBLStateInExecutionEnvironment({ env, expression: "pipoNum > 20 & pipoNum < 30", stateName: "pipo" }),
        eventStart: new Event_1.CCBLEvent({ eventName: "start", env, expressionFilter: "event.value > 0" }),
        eventFinish: new Event_1.CCBLEvent({ eventName: "finish", env, expressionFilter: "event.value == 27" })
    });
    it("Context is not activable", () => expect(context.getActivable()).toBe(false));
    it("Context is not activated", () => expect(context.getActive()).toBe(false));
    it("Start-Event activation has no effect on context", () => {
