...
class CCBLProgramObject {
    ...
    toHumanReadableProgram() {
        ...
        const contextToHumanReadable = context => {
            ...
            switch (context.type) {
                ...
                default:
                    throw `Unknow context type "${context.type}"`;
            }
        };
        const eventContextToHumanReadable = context => {
            return {
                contextName: context.contextName,
                eventSource: context.event.eventName,
                actions: context.channelActions.map(actionToHumanReadable)
            };
        };
        const eventTriggerToHumanReadable = evt => {
            return evt ? {
                eventName: evt.eventName,
                eventSource: evt.eventerSourceId,
                eventFilter: evt.expressionFilter
            } : undefined;
        };
        const stateContextToHumanReadable = context => {
            const relationships = {};
            context.parentOfAllenRelationships.forEach(rel => {
                switch (rel.type) {
                    case Allen_1.getAllenTypeName(AllenInterface_1.AllenType.During):
                        relationships.During = relationships.During || [];
                        relationships.During.push(...rel.children.map(contextToHumanReadable));
                        break;
                    case Allen_1.getAllenTypeName(AllenInterface_1.AllenType.Meet):
                        relationships.Meet = relationships.Meet || { contextsSequence: [], loop: 0 };
                        relationships.Meet.contextsSequence.push(...rel.children.map(contextToHumanReadable).filter(c => !!c));
                        break;
                    case Allen_1.getAllenTypeName(AllenInterface_1.AllenType.StartWith):
                        relationships.StartWith = relationships.During || [];
                        relationships.StartWith.push(...rel.children.map(contextToHumanReadable));
                        break;
                    case Allen_1.getAllenTypeName(AllenInterface_1.AllenType.EndWith):
                        relationships.EndWith = relationships.During || [];
                        relationships.EndWith.push(...rel.children.map(contextToHumanReadable));
                        break;
                    default:
                        throw `Unknown allen type "${rel.type}"`;
                }
            });
            return {
                contextName: context.contextName,
                ...
                actionsOnStart: context.actionsOnStart.map(actionToHumanReadable),
                actionsOnEnd: context.actionsOnFinish.map(actionToHumanReadable),
                actions: context.channelActions.map(actionToHumanReadable),
                allen: relationships
            };
        };
        const rootContextHumanReadable = stateContextToHumanReadable(this.rootContext.toJSON());
        const hrp = {
            dependencies: {
                import: {
    ...
    loadHumanReadableContextState(descr, context) {
        if (context === undefined) {
            ...
            context = new ContextState_1.CCBLContextState(Object.assign({}, contextConfig, { contextName: descr.contextName }));
    ...
    getSubProgramIdentifiedBy(instanceName) {
        ...
        for (const progName of progNames) {
            ...
            if (instance) {
                return Object.assign({}, instance, { programClassName: progName });
            }
        }
        return undefined;
    }
