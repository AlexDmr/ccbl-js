...
class CCBLProgramObject {
    ...
    loadHumanReadableProgram(descr, env, mapInputs) {
        ...
        emittersImport
            ...
            if (emitter) {
                ...
                const exprStr = mapInputs[name].expression;
                ...
                if (exprStr !== undefined) {
                    console.log(name, "mapped to expression:", exprStr, "valuated", expr.forceEvaluationOnce());
                }
    ...
    createLocalChannels(...channels) {
        const L = channels.map(({ name, channel, type }) => {
            if (!channel) {
                channel = Channel_1.getNewChannel();
    ...
    createLocalEventer(...eventers) {
        const L = eventers.map(({ name, eventer, type }) => {
            if (!eventer) {
                eventer = new Event_1.CCBLEvent({ eventName: name, env: this.getEnvironment() });
