...
import { cleanProgram, copyProgRef } from "./ProgramObjectInterface";
...
export class CCBLProgramObject {
    ...
    getCcblElements() {
        ...
        const processed = [];
        const toProcess = [this.rootContext];
        while (toProcess.length > 0) {
            const C = toProcess.pop();
            if (processed.indexOf(C) === -1) {
                const Cstate = C;
                if (Cstate !== this.rootContext && Cstate.rootOfProgramId !== undefined) {
                    const subProg = this.getProgramInstance(Cstate.rootOfProgramId);
                    res.subProgramInstances[Cstate.rootOfProgramId] = subProg.getCcblElements();
                }
                else {
                    processed.push(C);
                    for (const rel of C.getParentOfAllenRelationships()) {
                        toProcess.push(...rel.getChildren());
                    }
                }
            }
        }
        for (const C of processed) {
            switch (C.getType()) {
                case "CCBLContextEvent":
                    const Cevent = C;
                    res.eventContexts[Cevent.id] = Cevent;
                    for (const ae of Cevent.getChannelActionEvents()) {
                        res.eventActions[ae.id] = ae;
                    }
                    break;
                case "CCBLContextState":
                    const Cstate = C;
                    res.stateContexts[Cstate.id] = Cstate;
                    for (const as of Cstate.getChannelActionStates()) {
                        res.stateActions[as.id] = as;
                    }
                    for (const ae of [...Cstate.eventContextStart.getChannelActionEvents(), ...Cstate.eventContextEnd.getChannelActionEvents()]) {
                        res.eventActions[ae.id] = ae;
                    }
                    break;
                default:
                    throw `Unknown context type ${C.getType()}`;
            }
        }
        return res;
    }
    ...
    plugSubProgramAs(config) {
        ...
        try {
            ...
            console.error(`Error plugging subprogram ${programId} as ${asProgramId}`, "with map\n", mapInputs, "\n", err);
            const env = this.getEnvironment();
            env.getAllChannels().forEach(chan => console.log("\t ->", env.getNameOfChannel(chan)));
            throw `Error plugging subprogram: ${programId} as "${asProgramId}": ${err}`;
        }
        subprog.activate(true);
        const progVar = {};
        this.environment.registerProgInstance(asProgramId, progVar);
        subProgramInstances.push({
            as: asProgramId,
            mapInputs,
            prog: subprog,
    ...
    toHumanReadableProgram() {
        ...
        this.subPrograms.forEach((subP, Pname) => {
            subPrograms[Pname] = cleanProgram(subP.description);
        });
        ...
        const stateContextToHumanReadable = context => {
            ...
            if (relationships.During?.length === 0) {
                delete relationships.During;
            }
            if (relationships.EndWith?.length === 0) {
                delete relationships.EndWith;
            }
            if (relationships.StartWith?.length === 0) {
                delete relationships.StartWith;
            }
            if (relationships.Meet?.contextsSequence.length === 0 && relationships.Meet?.loop === undefined) {
                delete relationships.Meet;
            }
            const R = {
                type: "STATE",
                contextName: context.getContextName(),
        ...
        const hrp = {
            dependencies: {
                ...
                export: {
                    ...
                    emitters: this.outputEmittersId.map(id => ({ name: id, type: this.localChannels.get(id).type })),
        ...
        const hd = this.getHumanReadableDescription();
        if (hd?.name !== undefined) {
            hrp.name = hd.name;
        }
        if (hd?.description) {
            hrp.description = hd.description;
        }
        return cleanProgram(hrp);
    }
    getHumanReadableDescription() {
        return this.humanReadableDescription;
    }
    loadHumanReadableProgram(descr, env, mapInputs) {
        const wasActivated = !!this.rootChannel.getValueEmitter().get();
        this.activate(false);
        this.subPrograms.forEach(sp => {
            sp.instances.forEach(pi => {
        ...
        this.inputEmittersId = [];
        this.outputEmittersId = [];
        const eventerToBeDeleted = [...this.localEventers.keys()].filter(id => this.inputEventersId.indexOf(id) === -1);
        for (const evId of eventerToBeDeleted) {
            const ev = this.localEventers.get(evId);
            this.environment.unregisterCCBLEvent(ev.name);
            ev.eventer.dispose();
        }
        this.inputEventersId.forEach(ev => this.environment.unregisterCCBLEvent(ev));
        this.localEventers.clear();
        this.inputEventersId = [];
        this.outputEventersId = [];
        const elements = this.getCcblElements();
        for (const c of Object.values(elements.stateContexts)) {
            c.dispose();
        }
        this.rootContext = new CCBLContextState({
            contextName: `${this.name}__state`,
            environment: this.environment,
            rootOfProgramId: this.name,
            state: new CCBLStateInExecutionEnvironment({ stateName: `${this.name}__state`, env: this.environment, expression: "rootState" })
        });
        ...
        channelsImport
            ...
            throw `Channel ${name}/${mapInputs[name]} does not exist in imported environment`;
        } })
            .forEach(chanDescr => {
    ...
    loadContextOrProgram(allen, hostContext, descr) {
        ...
        if (progRef.programId) {
            const SP = this.plugSubProgramAs({
                ...
                as: progRef.as,
                description: progRef.description,
