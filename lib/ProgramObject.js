...
export class CCBLProgramObject {
    ...
    toHumanReadableProgram() {
        ...
        const stateContextToHumanReadable = context => {
            ...
            if (R.actionsOnStart?.length === 0) {
                delete R.actionsOnStart;
            }
            if (R.actionsOnEnd?.length === 0) {
                delete R.actionsOnEnd;
            }
            if (R.state === undefined) {
                delete R.state;
            }
            if (R.eventStart === undefined) {
                delete R.eventStart;
            }
            if (R.eventFinish === undefined) {
                delete R.eventFinish;
            }
            if (R.allen?.During === undefined && R.allen?.EndWith === undefined && R.allen?.StartWith === undefined && R.allen?.Meet === undefined) {
                delete R.allen;
            }
            return R;
        };
        ...
        if (Object.keys(subPrograms).length === 0) {
            delete hrp.subPrograms;
        }
        if (hrp.dependencies?.import?.channels?.length === 0) {
            delete hrp.dependencies?.import?.channels;
        }
        if (hrp.dependencies?.import?.emitters?.length === 0) {
            delete hrp.dependencies?.import?.emitters;
        }
        if (hrp.dependencies?.import?.events?.length === 0) {
            delete hrp.dependencies?.import?.events;
        }
        if (Object.keys(hrp.dependencies?.import ?? {}).length === 0) {
            delete hrp.dependencies?.import;
        }
        if (hrp.dependencies?.export?.channels?.length === 0) {
            delete hrp.dependencies?.export?.channels;
        }
        if (hrp.dependencies?.export?.emitters?.length === 0) {
            delete hrp.dependencies?.export?.emitters;
        }
        if (hrp.dependencies?.export?.events?.length === 0) {
            delete hrp.dependencies?.export?.events;
        }
        if (Object.keys(hrp.dependencies?.export ?? {}).length === 0) {
            delete hrp.dependencies?.export;
        }
        if (Object.keys(hrp.dependencies ?? {}).length === 0) {
            delete hrp.dependencies;
        }
        return hrp;
    }
