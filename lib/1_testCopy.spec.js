...
describe("Copy Domicube -> actions", () => {
    it("copy is OK", () => {
        ...
        expect(ProgramObjectInterface_1.stateActionsEquivalents(domicube_1.domicube.actions, actions)).toBe(true);
    });
});
...
describe("Copy eventsWithExpression", () => {
    it("copy of EndWith start event is OK", () => {
        ...
        expect(ProgramObjectInterface_1.eventEquivalent(context1.eventStart, context2.eventStart)).toBe(true);
    });
    it("copy of EndWith actions are OK", () => {
        const copyeventsWithExpression = ProgramObjectInterface_1.copyHumanReadableProgram(EventsWithExpression_1.eventsWithExpression);
        const context1 = EventsWithExpression_1.eventsWithExpression.allen.EndWith[0];
        const context2 = copyeventsWithExpression.allen.EndWith[0];
        expect(ProgramObjectInterface_1.stateActionsEquivalents(context1.actions, context2.actions)).toBe(true);
    });
    it("copy of EndWith is OK", () => {
        const copyeventsWithExpression = ProgramObjectInterface_1.copyHumanReadableProgram(EventsWithExpression_1.eventsWithExpression);
        expect(ProgramObjectInterface_1.contextEquivalent(EventsWithExpression_1.eventsWithExpression.allen.EndWith[0], copyeventsWithExpression.allen.EndWith[0])).toBe(true);
    });
    it("copy of During[3] is OK", () => {
        const copyeventsWithExpression = ProgramObjectInterface_1.copyHumanReadableProgram(EventsWithExpression_1.eventsWithExpression);
        const eq = ProgramObjectInterface_1.contextEquivalent(EventsWithExpression_1.eventsWithExpression.allen.During[3], copyeventsWithExpression.allen.During[3]);
        if (!eq) {
