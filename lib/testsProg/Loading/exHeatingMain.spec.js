...
describe("Loading Heating main", () => {
    ...
    it("should be possible to create the environment", () => {
        ...
        Lchan.forEach((vd) => {
            ...
            expect(env.get_Channel_FromId(vd.name)).toBe(chan);
        });
        Lemit.forEach((vd) => {
            const emitter = new CCBLEmitterValue(undefined);
            env.register_CCBLEmitterValue(vd.name, emitter);
            expect(env.get_CCBLEmitterValue_FromId(vd.name)).toBe(emitter);
        });
        Levts.forEach((vd) => {
            ...
            expect(env.getCCBLEvent(vd.name)).toBe(eventer);
        });
    });
    it("should be possible to load program", () => {
        prog.loadHumanReadableProgram(P, env, {});
        expect(progEquivalent(prog.getHumanReadableDescription(), P)).toBe(true);
    });
});
