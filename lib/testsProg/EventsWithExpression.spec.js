import { contextEquivalent, copyHumanReadableProgram, progEquivalent } from "../ProgramObjectInterface";
import { eventsWithExpression } from "./EventsWithExpression";
...
describe("Simulation for EventsWithExpression", () => {
    const P = copyHumanReadableProgram(eventsWithExpression);
    ...
    const EvtPlus = new CCBLEvent({
        eventName: "EvtPlus",
        env: sourceEnv
    });
    const EvtMinus = new CCBLEvent({
        eventName: "EvtMinus",
        env: sourceEnv
    });
    ...
    it("Loading program eventsWithExpression, human versions original and generated should be equivalent", () => {
        ...
        const eq = progEquivalent(P, CP, false);
        if (!eq) {
    ...
    it("EndWith should be equivalent", () => {
        ...
        expect(contextEquivalent(c1, c2, false)).toBe(true);
    });
    it("During[0] should be equivalent", () => {
        const c1 = P.allen.During[0];
        const c2 = CP.allen.During[0];
        const eq = contextEquivalent(c1, c2, false);
        if (!eq) {
