...
import { CcblProgramElements_to_JSON } from "../ProgramObjectInterface";
        ...
        try {
            switch (M.msg.type) {
                case "LoadRootProgram":
                    console.log("worker::LoadRootProgram");
                    ...
                    for (const C of prog.dependencies?.import?.channels ?? []) {
                        console.log("worker: \t- creating channel", C.name);
                    ...
                    for (const E of prog.dependencies?.import?.emitters ?? []) {
                        console.log("worker: \t- creating emitter", E.name);
                    ...
                    for (const EV of prog.dependencies?.import?.events ?? []) {
                        console.log("worker: \t- creating eventer", EV.name);
                        ...
                        chan.getValueEmitter().on(v => {
                            W.emit({
                                ...
                                res: {
                                    ...
                                    hasActiveAction: chan.hasActiveAction.get()
                                }
                            });
                        });
                    }
                    for (const E of prog.dependencies?.export?.emitters ?? []) {
                        ...
                        console.log("worker: \t: subscribe to emitter", E.name);
                        em.on(v => {
                    ...
                    for (const EV of prog.dependencies?.export?.events ?? []) {
                        ...
                        console.log("worker: \t: subscribe to eventer", EV.name);
                        ev.on(v => {
                    ...
                    response = {
                        type: "program update",
                        ...CcblProgramElements_to_JSON(elements)
                    };
                    break;
                case "SIMULATION":
                    console.log("worker: SIMULATION", M.msg.command);
                    clock.goto(Date.now());
                    ccblProg.activate(M.msg.command === "START");
                    ccblProg.UpdateChannelsActions();
                    if (M.msg.command === "START") {
                        clearInterval(clockIntervalSub);
                        clockIntervalSub = setInterval(() => {
                            clock.goto(Date.now());
                            ccblProg.UpdateChannelsActions();
                        }, 100);
                    }
                    ...
                    break;
                case "emitterUpdate":
                    console.log("worker: emitter update", M.msg.emitterId, "<-", M.msg.emitterValue);
