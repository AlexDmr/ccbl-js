import { getInstance } from "../ProgramObjectInterface";
export class CCBLProgramWorker {
    ...
    get currentElements() {
        return this.change.program.values.get("root");
    }
    getProgramElements(path) {
        if (this.currentElements) {
            const PI = getInstance(path, this.currentElements);
            return PI ? PI[PI.length - 1].elements : undefined;
        }
        return undefined;
    }
    getLastActionUpdateFor(actionId) {
        return this.change.action.values.get(actionId);
    }
    getLastContextUpdateFor(contextId) {
        return this.change.context.values.get(contextId);
    }
    getAccessPathFor(path) {
        return [...this.access.keys()].find(p => path.length === p.length && !p.find((s, i) => s !== path[i])) ?? path;
    }
    hasAccess(userId, path) {
        if (this.currentElements) {
            const realPath = this.getAccessPathFor(path);
            if (realPath) {
                const access = this.access.get(realPath);
                if (access.owners.indexOf(userId) >= 0)
                    return "OWNER";
                if (access.W.indexOf(userId) >= 0)
                    return "W";
                if (access.R.indexOf(userId) >= 0)
                    return "R";
            }
        }
        return "";
    }
    provideAccess(path, access) {
        if (this.currentElements) {
            const instancePath = getInstance(path, this.currentElements);
            if (instancePath) {
                const newPath = this.getAccessPathFor(path);
                this.access.set(newPath, access);
                return { type: "success", path: instancePath, access };
            }
            else {
                return { type: "error", reason: "There is no program instance for this path" };
            }
        }
        return { type: "error", reason: "There is loaded program" };
    }
