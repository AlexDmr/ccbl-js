...
const EventOr_1 = require("./EventOr");
const EventAnd_1 = require("./EventAnd");
const EventSeq_1 = require("./EventSeq");
...
describe("Event<string banco>:", () => {
    let event = new Event_1.CCBLEvent({
...
describe("EventOr( Event<boolean>, Event<string banco> ):", () => {
    let eventBool = new Event_1.CCBLEvent({ eventName: "eventBool", env });
    let eventBanco = new Event_1.CCBLEvent({
        eventName: "eventBanco",
        env,
        expressionFilter: `event.value == "banco"`
    });
    let eventOr = new EventOr_1.CCBLEventOr("eventOr", env, evt => evt);
...
describe("EventAnd( Event<boolean>, Event<boolean>, Event<string banco> ) in 1000ms:", () => {
    let eventBool1 = new Event_1.CCBLEvent({ eventName: "eventBool1", env });
    let eventBool2 = new Event_1.CCBLEvent({ eventName: "eventBool2", env });
    let eventBanco = new Event_1.CCBLEvent({
        eventName: "eventBanco",
        env,
        expressionFilter: `event.value == "banco"`
    });
    let eventAnd = new EventAnd_1.CCBLEventAnd("eventAnd", env, 1000, events => ({ value: "got all" }));
...
describe("EventAnd (eventBool1, eventBool1, eventBool1) in 1000ms:", () => {
    let eventBool1 = new Event_1.CCBLEvent({ eventName: "eventBool1", env });
    let eventAnd = new EventAnd_1.CCBLEventAnd("eventAnd", env, 1000, events => { return { value: "got all" }; });
...
describe("EventSeq( eventBool1, Event<string banco>, eventBool2 ) in 1000ms:", () => {
    ...
    let eventSeq = new EventSeq_1.CCBLEventSeq("eventSeq", env, 1000, events => { return { value: "got all" }; });
