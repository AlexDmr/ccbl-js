...
class Channel {
    constructor(valueEmitter) {
        ...
        this.foreCommit = false;
        channels.push(this);
        this.cbEmitter = (data) => {
            this.foreCommit = true;
        };
    ...
    ApplyConstraintsToValue(original) {
        ...
        if (constraints.length) {
            fctConstraint = x => constraints.reverse().reduce((result, f) => f.apply(result), x);
    ...
    shouldConsiderCommitting() {
        return this.dirty || this.foreCommit;
    }
    commit() {
        let valueChanged = false;
        if (this.foreCommit || (this.dirty && this.configChanged())) {
            this.foreCommit = false;
            ...
            if (emitterValue) {
                let newValue = this.ApplyConstraintsToValue(emitterValue.get());
...
function commitStateActions(...channels) {
    if (channels.length === 0) {
        channels = Channels.filter(chan => chan.shouldConsiderCommitting());
    }
    while (channels.length) {
        channels.forEach(chan => {
            ...
            if (nextConf.emitterValue instanceof CCBLExpressionInExecutionEnvironment_1.CCBLExpressionInExecutionEnvironment) {
                ...
                doCommit = emitters.reduce((acc, e) => acc && channels.indexOf(e) === -1, true);
            }
            if (doCommit) {
                chan.commit();
            }
